...
static struct ListNode * GetNodeAtIndex(const struct List *list,
                                        size_t index)
{
	assert(list);
	
	struct ListNode *node = GetFirstNode(list);
	
	for (size_t i = 0; i < index; ++i) {
		assert(node != GetTailNode(list));
...
static struct ListNode * ContainsDataStep(const struct List *list,
                                      ...
                                      Data_comp_func_t comp_func)
{
	...
	return ContainsDataStep(list, data, comp_func, 1);
}
...
static struct ListNode * AddNodeAtIndex(struct List *list,
                                        struct ListNode *node,
                                        size_t index)
{
	assert(list);
	...
	struct ListNode *ref = GetHeadNode(list);
	
	for (size_t i = 0; i < index; ++i) {
		ref = GetNodeNext(ref);
	}
	
	return AddNodeAfter(list, node, ref);
}
...
static struct ListNode * AddDataAtIndex(struct List *list,
                                        const struct Data *data,
                                        size_t index)
{
	assert(list);
	...
	return AddNodeAtIndex(list, CreateNode(data), index);
}
                                        ...
                                        size_t stepSize)
{
	...
	return ContainsDataStep(list, data, comp_func, stepSize);
}
                                         ...
                                         size_t index)
{
	...
	return AddNodeAtIndex(list, copy, index);
}
                                      ...
                                      size_t index)
{
	...
	return AddDataAtIndex(list, data, index);
}
